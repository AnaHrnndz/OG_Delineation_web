...
# from  og_delineation import load_tree_web, load_taxonomy, load_reftree, load_taxonomy_counter, run_preanalysis_annot_tree, get_og_info #taxlev2ogs_annotated_tree#, run_load_annotated_tree
# from  og_delineation import run_preanalysis, run_outliers_and_scores, run_dups_and_ogs, run_clean_properties, get_newick,  get_taxlevel2ogs, add_ogs_up_down, get_analysis_parameters, flag_seqs_out_og, prune_tree
...
def upload_annotated_tree(tree, name_tree, reftree, taxonomy_counter, user_taxo, taxonomy_type, taxonomy_db):
    ...
    t , sp_set, total_mems_in_tree, SPTOTAL, taxid_dups, names_ogs = og_delineation.run_preanalysis_annot_tree(tree, name_tree)
    print('tree upload')
    ...
    for taxo in taxo_stats_info:
        ...
        glob_taxlev2ogs[taxid]['ogs_names'] = ogs
    ...
    prune_t = og_delineation.prune_tree(dup_tree, total_mems_in_ogs)
    ...
    t_prune = og_delineation.get_newick(prune_t, all_props)
...
def run_upload(tree, name_tree, reftree, user_counter, user_taxo, taxonomy_type, midpoint):
    ...
    t = og_delineation.load_tree_web(tree= tree)
    ...
    #Pre-analysis: rooting, annotate tree, etc
        t , sp_set, total_mems_in_tree, SPTOTAL, user_props = og_delineation.run_preanalysis(t, name_tree, taxonomy_db, midpoint)
...
def run_analysis():
    ...
    t =  og_delineation.run_outliers_and_scores(t, outliers_node, outliers_reftree, sp_loss_perc, so_cell_org, so_arq, so_bact, so_euk, taxonomy_counter, taxonomy_db, SPTOTAL, reftree, inherit_outliers)

    total_mems_in_ogs = set()
    t, total_mems_in_ogs, ogs_info, taxid_dups_og = og_delineation.run_dups_and_ogs(t, outliers_node, outliers_reftree, sp_loss_perc, so_cell_org, so_arq, so_bact, so_euk, taxonomy_db, total_mems_in_tree)


    t, ogs_info = og_delineation.add_ogs_up_down(t, ogs_info)

    #Extended OGs at each taxid level
    taxlev2ogs =  og_delineation.get_taxlevel2ogs(t, taxid_dups_og, total_mems_in_ogs,taxonomy_db)
    ...
    if 'aln_path' in current_data.keys():
        ...
        og_delineation.clean_folder(pathout, aln_path)
